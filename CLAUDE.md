...
- `absent(up{...} == 1)` is **not** an instance of the rule above — it is the fix for it. `absent()` fires *because* the series vanished, which is the failure mode `== 0` misses. Its actual limitation is different: it is cluster-wide (fires only once every matching target is gone) and cannot name the failing instance.
